...
describe('directives', () => {
  describe('asyncAppend', () => {
    it('should render an AsyncIterable value', async () => {
      ...
      assert((await render(result)) === 'some async text');
    });
    it('should render a mapped AsyncIterable value', async () => {
      ...
      assert((await render(result)) === 'some 0-ASYNC1-TEXT');
    });
  });
  ...
  describe('asyncReplace', () => {
    it('should render an AsyncIterable value', async () => {
      ...
      assert((await render(result)) === 'some async');
    });
    it('should render a mapped AsyncIterable value', async () => {
      const result = h`some ${asyncReplace(createAsyncIterable(['async', 'text']), (v, index) => {
        return `${index}-${v.toUpperCase()}`;
      })}`;
      assert((await render(result)) === 'some 0-ASYNC');
    });
  });
  ...
  describe('cache', () => {
    it('should render a cached value', async () => {
      ...
      assert((await render(result)) === 'some text');
    });
  });
  ...
  describe('guard', () => {
    ...
    it('should render a guarded array value', async () => {
      ...
      assert((await render(result)) === 'some 123');
    });
  });
  ...
  describe('if-defined', () => {
    it('should render an attribute value if defined', async () => {
      ...
      assert((await render(result)) === '<div class="hi"></div>');
    });
    it('should not render an attribute value if undefined', async () => {
      const className = undefined;
      const result = h`<div class="${ifDefined(className)}"></div>`;
      assert((await render(result)) === '<div ></div>');
    });
  });
  ...
  describe('repeat', () => {
    it('should render an array of values', async () => {
      ...
      assert((await render(result)) === '<ul><li>0: 1</li><li>1: 2</li><li>2: 3</li></ul>');
    });
  });
  ...
  describe('styleMap', () => {
    ...
    it('should include style properties', async () => {
      ...
      assert((await render(result)) === '<div style="color:red;border:1px solid black;"></div>');
    });
  });
  ...
  describe('unsafe-html', () => {
    it('should render unescaped value', async () => {
      ...
      assert((await render(result)) === "<p>hey! it's dangerous! <script>boom!</script></p>");
    });
  });
  ...
  describe('custom', () => {
    it('should allow writing custom directives', async () => {
      ...
      assert((await render(result)) === '<p>custom&#x27;s</p>');
    });
    it('should give correct tagName', async () => {
      ...
      await render(result);
      assert(actualTagName === 'my-static-element');
    });
